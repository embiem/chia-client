...
class FullNode extends RpcClient {
  ...
  ): Promise<BlockRecordsResponse> {
    return this.request<BlockRecordsResponse>("get_block_records", {
      start,
      end,
    });
  }
  ...
  public async getAllMempoolTxIds(): Promise<TransactionIdsResponse> {
    return this.request<TransactionIdsResponse>("get_all_mempool_tx_ids", {});
  }

  public async getAllMempoolItems(): Promise<MempoolItemsResponse> {
    return this.request<MempoolItemsResponse>("get_all_mempool_items", {});
  }

  public async getMempoolItemByTxId(
    txId: string
  ): Promise<MempoolItemResponse> {
    return this.request<MempoolItemResponse>("get_mempool_item_by_tx_id", {
      tx_id: txId,
    });
  }
}
